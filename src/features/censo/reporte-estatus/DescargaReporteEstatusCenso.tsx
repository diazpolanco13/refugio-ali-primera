...
} from "./ReporteEstatusCensoRedPdf";
...
function nombreArchivo(ts: number): string {
  ...
  return `estatus-censo-red-${yyyy}${mm}${dd}-${hh}${mi}.pdf`;
}

export function DescargaReporteEstatusCenso({
  datos,
}: {
      ...
      {({ url, loading, error }) => {
        ...
        return (
          <Button asChild size="sm" variant="default" className="h-8 gap-1.5">
            <a href={url} download={nombreArchivo(datos.generadoTs)} target="_blank" rel="noreferrer">
