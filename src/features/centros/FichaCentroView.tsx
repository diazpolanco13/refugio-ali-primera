...
import { normalizarCentro, poblacionCentro } from "@/domain/centrosTransitorios";
...
/** KPIs compactos para la pestaña Resumen. */
function KpisResumen({ centro }: { centro: CentroTransitorio }) {
  ...
  const items = [
    { etiqueta: "Refugiados", valor: refugiados, clase: "text-sky-300" },
    { etiqueta: "Familias", valor: analisis.familias },
    { etiqueta: "Personal", valor: analisis.personal, clase: "text-violet-300" },
    { etiqueta: "Total logístico", valor: analisis.personasLogistica, clase: "text-emerald-300" },
  ];
  return (
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background text-foreground">
      ...
      >
        ...
        <div className="shrink-0 border-b border-border bg-background/95 px-2 sm:px-3">
          <TabsList
            variant="line"
            className="mx-auto h-auto w-full max-w-5xl justify-start gap-0 overflow-x-auto rounded-none bg-transparent p-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            {SECCIONES.map((s) => (
              <TabsTrigger
                key={s.id}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          <div className="mx-auto w-full max-w-5xl p-3 pb-[max(1rem,env(safe-area-inset-bottom))] sm:p-4">
            <TabsContent value="resumen" className="mt-0 space-y-4">
              ...
              {c.notas?.trim() && <SeccionNotasCentro centro={centro} />}
            </TabsContent>
