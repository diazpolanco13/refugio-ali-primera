-- terreno_centro: expone cuerpo (institución responsable) y unidad SEBIN
-- de revista del campamento para mostrarlos en /terreno ANTES de pedir la
-- cédula. Así el operador entiende que esos datos son del centro, no suyos.
...
language sql stable security definer set search_path = public as $$
  ...
  select
    ...
    coalesce(nullif(trim(c.data->>'cuerpo'), ''), '') as cuerpo,
    coalesce(nullif(trim(c.data #>> '{supervision,unidad_sebin}'), ''), '') as unidad,
...
-- terreno_centro DEBE seguir abierta a anon (portal /terreno sin sesión).
