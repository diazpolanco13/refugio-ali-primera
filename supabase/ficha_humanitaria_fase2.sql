-- Ficha humanitaria completa — Fase 2
-- Salud, habilidades, documentación, seguimiento, residencia ampliada

-- ============================================================================
-- refugiados — bloques jsonb extensos
-- ============================================================================
alter table public.refugiados
  add column if not exists salud jsonb not null default '{}'::jsonb,
...
-- ============================================================================
-- alojamientos_refugiados — seguimiento de caso
-- ============================================================================
alter table public.alojamientos_refugiados
  add column if not exists seguimiento jsonb not null default '{}'::jsonb,
  add column if not exists destino_egreso text not null default '';

-- ============================================================================
-- residencias_afectadas — tenencia y pérdidas materiales
-- ============================================================================
alter table public.residencias_afectadas
  add column if not exists tipo_tenencia text not null default ''
    check (tipo_tenencia in ('', 'propia', 'alquilada', 'prestada', 'ocupacion', 'otro')),
  add column if not exists perdio_todo boolean not null default false,
  add column if not exists perdidas_materiales jsonb not null default '[]'::jsonb;

-- ============================================================================
-- RPC upsert_residencia_afectada — ampliado con tenencia y pérdidas
-- ============================================================================
...
-- Re-grant con nueva firma (drop old overloads if needed)
