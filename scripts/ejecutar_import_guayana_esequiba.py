#!/usr/bin/env python3
"""Ejecuta la importación de GUAYANA ESEQUIBA.TXT vía Supabase REST (censo_registrar)."""
...
# Cédulas ya presentes en la red (prechequeo MCP) → importar sin documento
DOCS_EN_RED = {"26473786"}
...
def load_importer():
    spec = importlib.util.spec_from_file_location(
        "importar_censo_guayana_esequiba",
        ROOT / "scripts" / "importar_censo_guayana_esequiba.py",
    )
...
def rpc_registrar(url: str, key: str, centro_id: str, funcionario: dict, registro: dict):
    payload = {
...
def main():
    imp = load_importer()
    url, key = load_env()
    _, ok, omitidos, reasignados = imp.preparar(docs_en_red=DOCS_EN_RED)
    ...
    for reg in ok:
        ...
        try:
            ...
            # Si cédula ya existe en red, reintentar sin documento
            if "cédula" in detail.lower() or "cedula" in detail.lower() or "duplicad" in detail.lower():
                payload2 = dict(payload)
                payload2["tipo_doc"] = ""
                payload2["documento"] = ""
                try:
                    reg_id = rpc_registrar(url, key, imp.CENTRO_ID, imp.FUNCIONARIO, payload2)
                    resultados["ok"].append(
                        {
                            "fila": reg["fila"],
                            "nombre": nombre,
                            "id": reg_id,
                            "nota": "reintentado sin documento por cédula en red",
                            "cedula_original": reg.get("documento"),
                        }
                    )
                    print(f"OK (sin doc) fila {reg['fila']}: {nombre}", file=sys.stderr)
                    continue
                except Exception as exc2:  # noqa: BLE001
                    detail = f"{detail} | retry: {exc2}"
    ...
    resumen = {
        ...
        "centro_nombre": "Complejo Guayana Esequiba",
        ...
        "detalle_errores": resultados["error"][:50],
    }
