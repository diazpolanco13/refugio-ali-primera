#!/usr/bin/env python3
"""Ejecuta la importación de eduando_crema.txt llamando censo_registrar vía Supabase REST."""
...
def load_importer():
    spec = importlib.util.spec_from_file_location(
        "importar_censo_eduardo_crema",
        ROOT / "scripts" / "importar_censo_eduardo_crema.py",
    )
...
def preparar_registros(imp):
    texto = imp.ARCHIVO.read_text(encoding="utf-8")
    registros = []
    for i, linea in enumerate(texto.splitlines(), start=1):
        reg = imp.parse_linea(linea, i)
        if reg:
            registros.append(reg)
    registros, reasignados = imp.resolver_cedulas_duplicadas(registros)
    imp.vincular_jefes_familia(registros)
    ok, omitidos = imp.deduplicar(registros)
    ok = imp.ordenar_para_importacion(ok)
    return ok, omitidos, reasignados


def main():
    imp = load_importer()
    url, key = load_env()
    ok, omitidos, reasignados = preparar_registros(imp)
