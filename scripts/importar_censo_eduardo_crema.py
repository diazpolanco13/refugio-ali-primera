#!/usr/bin/env python3
"""Importa planilla censo/eduardo_crema.txt al censo de centro-27 (UEN Eduardo Crema) vía SQL."""
...
CENTRO_ID = "centro-27"
ARCHIVO = Path("/opt/refugio-ali-primera/censo/eduardo_crema.txt")
FUNCIONARIO = {
    "jerarquia": "Sistema",
...
def parse_linea(linea: str, nro: int) -> dict | None:
    ...
    while len(cols) < 10:
        cols.append("")
    _, cod_fam, cedula, nombre, apellido, parentesco, sexo, _fecha, edad_raw, _grupo = cols[:10]
...
def registro_payload(reg: dict) -> dict:
    payload = {k: v for k, v in reg.items() if k not in ("fila", "documento_norm", "cod_fam", "parentesco", "es_jefe")}
