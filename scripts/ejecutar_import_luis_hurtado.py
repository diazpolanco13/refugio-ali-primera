#!/usr/bin/env python3
"""Ejecuta la importación de luis_hurtado_higuera.txt vía Supabase REST (censo_registrar)."""
...
def load_importer():
    spec = importlib.util.spec_from_file_location(
        "importar_censo_luis_hurtado",
        ROOT / "scripts" / "importar_censo_luis_hurtado.py",
    )
...
def main():
    imp = load_importer()
    url, key = load_env()
    ok, omitidos = preparar_registros(imp)
    ...
    resumen = {
        ...
        "centro_nombre": "UE Luis Hurtado (UEN Luis Hurtado Higuera)",
